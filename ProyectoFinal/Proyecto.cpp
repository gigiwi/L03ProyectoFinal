...
int main()
{
	...
	AgaveTexture = Texture("Textures/fuego2.png");
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//AEOLIPILE
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(20.0f, 5.0f, 1.5f));
		modelaux = model;
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		fuego += 0.2f * deltaTime;
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 1.2f, 0.0f));

		float escalaY = 1.0f + 0.5f * sin(fuego);
		model = glm::scale(model, glm::vec3(2.0f, escalaY * 2.0f, 2.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glEnable(GL_BLEND);
		...
		// desactivar blending después de usarlo
